...
@register
class EffHybridEncoder(nn.Module):
    def __init__(
        self,
        in_channels=[512, 1024, 2048],
        feat_strides=[8, 16, 32],
        ...
        self.fpn_strides = list(set(feat_strides) | set(encoder_feat_strides))
        self.num_overlap = int(
            len(self.feat_strides)
            + len(self.encoder_feat_strides)
        ...
        self.lateral_convs = nn.ModuleList()
        self.fpn_blocks = nn.ModuleList()
        for _ in range(len(self.fpn_strides) - 1, 0, -1):
            self.lateral_convs.append(
                ConvNormLayer(hidden_dim, hidden_dim, 1, 1, act=act)
            )
            self.fpn_blocks.append(
        ...
        self.downsample_convs = nn.ModuleList()
        self.pan_blocks = nn.ModuleList()
        for _ in range(len(self.fpn_strides) - 1):
            self.downsample_convs.append(
                ConvNormLayer(hidden_dim, hidden_dim, 3, 2, act=act)
            )
            self.pan_blocks.append(
    ...
    def forward(self, feats):
        ...
        for i, encoder in enumerate(self.encoders):
            if self.num_overlap - i < 1:
                proj_feats.append(F.max_pool2d(proj_feats[-1], 2, 2))
        ...
        # broadcasting and fusion
        inner_outs = [proj_feats[-1]]
        for idx in range(len(self.fpn_strides) - 1, 0, -1):
            feat_high = inner_outs[0]
            feat_low = proj_feats[idx - 1]
            feat_high = self.lateral_convs[len(self.in_channels) - 1 - idx](feat_high)
            inner_outs[0] = feat_high
        ...
        outs = [inner_outs[0]]
        for idx in range(len(self.fpn_strides) - 1):
            feat_low = outs[-1]
            feat_high = inner_outs[idx + 1]
            downsample_feat = self.downsample_convs[idx](feat_low)
            out = self.pan_blocks[idx](
