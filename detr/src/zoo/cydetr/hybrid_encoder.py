...
__all__ = ["HybridEncoder", 'CYHybridEncoder']
...
class CSPRepLayer_next(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(CSPRepLayer_next, self).__init__()
        hidden_channels = int(out_channels * expansion)
        self.conv1 = ConvNormLayer(
            in_channels, hidden_channels, 1, 1, bias=bias, act=act
        )
        ...
        self.bottlenecks = nn.Sequential(
            *[
                ConvNeXtV2Block(hidden_channels)
                for _ in range(num_blocks)
            ]
        )
        if hidden_channels != out_channels:
...
@register
class CYHybridEncoder(nn.Module):
    def __init__(
        self,
        in_channels=[512, 1024, 2048],
        feat_strides=[8, 16, 32],
