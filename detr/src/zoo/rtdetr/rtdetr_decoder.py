...
class MSDeformableAttention_IdB(nn.Module):
    def __init__(
        self,
        embed_dim=256,
        num_heads=8,
        ...
        Multi-Scale Deformable Attention Module
        """
        super(MSDeformableAttention_IdB, self).__init__()
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.num_levels = num_levels
        self.num_points = num_points
        ...
        self.attention_weights = nn.Linear(embed_dim, self.total_points)
        self.value_proj = nn.Conv2d(embed_dim, embed_dim, kernel_size=7, padding=3)
        self.output_proj = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(
        ...
        value_list = []
        before = 0
        for spatial_shape in value_spatial_shapes:
            value_list.append(
                value[:, before : before + spatial_shape[0] * spatial_shape[1], :]
                .permute(0, 2, 1)
                .reshape(bs, -1, spatial_shape[0], spatial_shape[1])
            )
            before += spatial_shape[0] * spatial_shape[1]
            value_list[-1] = self.value_proj(value_list[-1])
            value_list[-1] = (
                value_list[-1].reshape(bs, self.embed_dim, -1).permute(0, 2, 1)
            )
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        # cross attention
        self.cross_attn = MSDeformableAttention_IdB(d_model, n_head, n_levels, n_points)
...
@register
class RTDETRTransformer2(nn.Module):
    __share__ = ["num_classes"]
    ...
    def __init__(
        ...
        print("RTDETRTransformer2")
        super(RTDETRTransformer2, self).__init__()
        assert position_embed_type in [
            "sine",
            "learned",
        ], f"ValueError: position_embed_type not supported {position_embed_type}!"
