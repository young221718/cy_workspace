...
class CYDecoder2(nn.Module):
    ...
    def __init__(
        ...
        super(CYDecoder2, self).__init__()
        ...
        self.dec_score_head = nn.ModuleList(
            [nn.Linear(hidden_dim, num_classes + 1) for _ in range(num_decoder_layers)]
        )
    ...
    def _get_decoder_input(
        ...
        _, topk_ind = torch.topk(
            F.softmax(enc_outputs_class)[:, :, :-1].max(-1).values,
            self.num_queries,
            dim=1,
        )
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord):
        ...
        return [
            {"pred_logits": b, "pred_boxes": c}
            for b, c in zip(outputs_class, outputs_coord)
        ]
